...
bool CStair::Collide(CObject* other)
{
	...
	if (p)
	{
		...
		else
		{
			...
			{
				double a = p->GetVelocity().x;
				double b = p->GetVelocity().y;

				double c = 2 * ((dir * 1 / sqrt(2)) * a - (1 / sqrt(2)) * b) * (dir * 1 / sqrt(2)) - a;  // 반사각 계산
				double d = 2 * ((dir * 1 / sqrt(2)) * a - (1 / sqrt(2)) * b) * (-(1 / sqrt(2))) - b;
				p->GetVelocity().x = 0;
				p->GetVelocity().y = 0.5*d;
	...
	if (p)
	{
		...
		{ 
				...
				else if (p->GetHurt())
				{
					...
					double c = 2 * ((dir * 1 / sqrt(2)) * a - (1 / sqrt(2)) * b) * (dir * 1 / sqrt(2)) - a;
					double d = 2 * ((dir * 1 / sqrt(2)) * a - (1 / sqrt(2)) * b) * (-(1 / sqrt(2))) - b;
					p->GetVelocity().x = 0;
					p->GetVelocity().y = 0.5 * d;
