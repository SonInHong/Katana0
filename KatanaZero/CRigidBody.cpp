...
void CRigidBody::Update()
{
	...
	m_Accelaration.y = m_GivenAccel.y + m_Force.y / m_Mass;
	if(m_Gravity)
		m_Accelaration.y += Gravity_Accelaration;
	...
	if (m_Attack == false)
	{
		...
		if (m_FallStair && abs(m_Velocity.x) > m_MaxXFallspeed)
	...
	//디버깅용
	wchar_t _Buffer[250];
	swprintf_s(_Buffer,  L"RecentFloor: (%f, %f), DeltaTime: %f, Velocity.x: %f, Velocity.y: %f, OnGround: %d, OnStair: %d, OnJump: %d, OnWall: %d, WallGrab: %d, Roll: %d, Flip:%d. Attack:%d, Hurt:%d",
		RFpos.x, RFpos.y, TimeMgr::Create()->dt(), m_Velocity.x, m_Velocity.y, m_OnGround, m_OnStair, m_Jump, m_OnWall, m_WallGrab, m_Roll, m_Flip, m_Attack, m_Hurt);
